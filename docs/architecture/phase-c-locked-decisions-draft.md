...
**Status: DRAFT — Requires architect approval before AVC authoring**
...
| User Verb | Relation Type Class | Rationale |
|-----------|-------------------|-----------|
| `has` (Tier 1 bare property) | `fandaws:relationType/has_part` (mereological) | "Has" is the default verb for part-whole. Mereological RECC applies. |
| Any BFO-matched verb (Tier 2A) | Determined by the BFO property's domain/range | The BFO property's declared domain and range determine which relation type class schema applies. |
| Unresolved verbs (Tier 1 with no BFO match) | No relation type class | Tier 1 bare properties have no RECC. They compile to the execution lane with `fandaws:objectProperty/{verb}` but carry no structural conformance restriction. This is by design — RECC enforcement requires knowing the relation type. |
...
|----------|-------------|--------|
| C-1 | Phase split: C1 (internal lifecycle) + C2 (external RECC) | DRAFT |
| C-2 | Retraction cascade via sourceCanonical, not hierarchy | DRAFT |
| C-3 | Confidence defaults: conversational=1.0, resolved=1.0, released=0.7, imported=N/A | DRAFT |
| C-4 | TentativeGraph = boolean flag, not separate structure | DRAFT |
| C-5 | Three stale detection trigger scopes: artifact, type, full | DRAFT |
| C-6 | Relation type class schemas: hardcoded seed set for Phase C | DRAFT |
| C-7 | Quarantine store: separate _quarantineStore Map on StateAdapter | DRAFT |

All decisions are DRAFT pending architect approval. Once approved, they become locked and the AVC bundles (C1 and C2) are authored against them.
