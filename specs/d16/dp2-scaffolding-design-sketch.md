...
- **DP-2.2** — **LANDED 2026-04-24.** Modules shipped: `src/core/d16/canonical-serialization.js` (JCS), `axiom-dictionary.js`, `dependency-graph.js` (per X1 memo), `provenance-builder.js`, `explanation-builder.js` + production composition on the writer. 94 new unit tests pass (JCS + dictionary + graph + builders). 8 Band 6/3/5 AVC scenarios now pass: all 4 `verifyDP2Conformance` disposition scenarios, `dp2-provenance-iteration-history`, `dp2-axiom-dictionary-deduplication`, Band 3 `evidence-inconsistent-override-path`, Band 5 `notapplicable-provenance-fields`.
...
- **DP-2.3.1 per-round hashing** — **LANDED 2026-04-24.** Module `src/core/d16/reproducibility-hash.js` shipped with `computePerRoundHash` + `buildProductionReproducibilityHash`. Session-hash registry (Forward-Flag Item 2) extended onto `bfo-signature-cache.js` per SME routing — closes the cache-mutation-between-write-and-read gap. V3 (F1 immediate-predecessor JSDoc + multi-step cascade test), V5 (OERS accumulate-all-before-throw + source ontology capture), V4 (applyMutationSequence must-compute fields per SME scoping) all folded in. AVC scenario `dp2-reproducibility-hash-per-round-and-final` passing (Final Hash remains scaffold until DP-2.3.2).
- **DP-2.3.2 Final Hash + F4 bundle v5** — **LANDED 2026-04-24.** Module `src/core/d16/session-config-snapshot.js` (frozen config snapshot with SessionConfigMutationError guard) + `computeFinalHash` + `finalizeCanonicalRecord` (I2b activation — removes `_scaffold` sentinel). `src/core/d16/record-persistence.js` helper wires chokepoint + adapter persist for F4 audit discovery. Bundle v4 → v5 with new scenario `dp2-writepath-chokepoint-exclusivity` (static + runtime audit, both pass). AVC scenarios `dp2-reproducibility-cross-session` (three-case regression guard: baseline identity, drift-guard on excluded fields, positive discrimination on IN fields) + `dp2-writepath-chokepoint-exclusivity` both passing. D1.6 AVC: 69/70 scenarios passing.
